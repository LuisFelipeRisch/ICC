...
int findPivot(SistLinear_t *SL, int currPivotPositionLine)
{ 
  int maxPivotPositionLine = currPivotPositionLine; 
  int currPivotPositionColumn = currPivotPositionLine; 

  for (int i = currPivotPositionLine; i < SL->n - 1; i++)
    if(SL->A[i + 1][currPivotPositionColumn] > SL->A[maxPivotPositionLine][currPivotPositionColumn])
      maxPivotPositionLine = i + 1;
  return maxPivotPositionLine; 
}

void exchangeLines(SistLinear_t *SL, int firstLine, int secondLine)
{
  real_t *auxLine = (real_t *) malloc(SL->n * sizeof(real_t)); 
  
  for (int i = 0; i < SL->n; i++)
  {
    auxLine[i] = SL->A[firstLine][i]; 
    SL->A[firstLine][i] = SL->A[secondLine][i]; 
    SL->A[secondLine][i] = auxLine[i]; 

    auxLine[i] = SL->b[firstLine]; 
    SL->b[firstLine] = SL->b[secondLine]; 
    SL->b[secondLine] = auxLine[i];
  } 
